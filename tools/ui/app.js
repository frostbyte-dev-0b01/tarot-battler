...
// Auto-generated by tools/ui/build-replays.sh: a plain array of the team
// filenames bundled in sample-data/teams/. A static page can't enumerate the
// directory itself, so this manifest tells the roster which teams to load.
const teamsManifestPath = "./sample-data/teams.json";
const teamsDir = "./sample-data/teams/";
...
async function seedRosterFromBundled() {
  ...
  try {
    const manifest = await fetchTeamJson(teamsManifestPath);
    for (const entry of manifest) {
      // Manifest entries are filenames; tolerate the legacy { file } shape too.
      const file = typeof entry === "string" ? entry : entry?.file;
      if (!file) {
        continue;
      }
      try {
        add(await fetchTeamJson(`${teamsDir}${file}`));
      } catch {

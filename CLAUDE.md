...
- **Rule system:** Each character has up to 5 ordered priority rules. Rule groups are `SelfChar`, `Companion`, `Target`, and `World`. `Companion` means any adjacent ally and does not imply that same companion becomes the ability target. Rules are evaluated top to bottom; the first satisfied rule fires, otherwise the character uses `Basic Attack` (which also generates 1 mana pip). World queries currently support live `ally_count`, `enemy_count`, and step-based `tick_count`.
